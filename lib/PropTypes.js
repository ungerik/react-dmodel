...
// Lightweight version of react's internal original
function checkPrimitiveType(expectedType, props, propName, componentName, location, propFullName) {
	const propValue = props[propName];
	const propType = typeof propValue;
	if (propType !== expectedType) {
		return new Error(
			`Invalid ${location} \`${propFullName}\` of type ` +
			`\`${propType}\` supplied to \`${componentName}\`, expected ` +
			`\`${expectedType}\`.`
		);
	}
	return null;
...
function validNumber(props, propName, componentName, location, propFullName) {
	const err = checkPrimitiveType("number", props, propName, componentName, location, propFullName);
	if (err) {
		return err;
	}
...
function numberOrNull(props, propName, componentName, location, propFullName) {
	...
	return checkPrimitiveType("number", props, propName, componentName, location, propFullName);
}
