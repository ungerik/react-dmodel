...
function uniqueKey() {
	uniqueKeyCounter++;
	return "dmodel" + uniqueKeyCounter;
}
...
const mapping = {
	...
	Float: (element, getValue, setValue) => {
		...
		if (props.style.slider) {
			return <Slider {...{...props.style.slider, ...props}} label={getLabel(props)} key={props.name} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
		}
	...
	Int: (element, getValue, setValue) => {
		const props = element.props;
		if (props.style.slider) {
			return <Slider {...{...props.style.slider, ...props}} label={getLabel(props)} integer key={props.name} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
		}
