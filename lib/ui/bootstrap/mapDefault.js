...
export default function mapDefault(element, getValue, setValue, inputProps = {}, parents = [], refCallback = () => undefined, childKey = "parent") {
...
const mapping = {
	...
	Text: (element, getValue, setValue, inputProps, parents, refCallback, key) => {
		const props = element.props;
		if (props.multiLine) {
			return <Input {...{...inputProps, ...props}} type="textarea" ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} value={getValue(props.name)} onChange={event => setValue(props.name, event.target.value)} />;
		} else {
			return <Input {...{...inputProps, ...props}} type="text" ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} value={getValue(props.name)} onChange={event => setValue(props.name, event.target.value)} />;
		}
	},
