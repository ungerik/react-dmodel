...
const mapping = {
	DataModel: (element, getValue, setValue, extraProps, parents, refCallback) => {
		...
		const mappedChildren = asArray(props.children).map((child, childKey) => mapDefault(child, getValue, setValue, extraProps, childParents, refCallback, childKey));
	...
	Float: (element, getValue, setValue, extraProps, parents, refCallback, key) => {
		const props = element.props;
		if (props.children) {
			return React.cloneElement(props.children, {...{getValue, setValue, parents, refCallback, key}, ...extraProps});
		}
		if (props.style.slider) {
			return <Slider {...{...props.style.slider, ...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
		}
		return <FloatInput {...{...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} addonAfter={props.unit} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
	},
	...
	Int: (element, getValue, setValue, extraProps, parents, refCallback, key) => {
		...
		if (props.style.slider) {
			return <Slider {...{...props.style.slider, ...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} integer value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
		}
		return <IntInput {...{...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} addonAfter={props.unit} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
	},
	...
	Uint: (element, getValue, setValue, extraProps, parents, refCallback, key) => {
		...
		return <UintInput {...{...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} addonAfter={props.unit} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
	},
	...
	Text: (element, getValue, setValue, extraProps, parents, refCallback, key) => {
		...
		return <TextInput {...{...extraProps, ...props}} ref={refCallback} key={key} label={getLabel(props)} placeholder={props.style.placeholder} value={getValue(props.name)} onChange={value => setValue(props.name, value)} />;
	},
		...
		return (
			<div key={key} className="form-group dmodel-enum">
				...
				{props.style.dropdown ?
					<div>
						<DropdownButton {...extraProps} id={label} title={props.optionsPrefix + translate(value) + props.optionsPostfix}>
							{
				...
				:
					<Nav {...extraProps} bsStyle='pills' stacked={props.style.stacked} activeKey={value} onSelect={key => setValue(props.name, key)}>
						{
							props.options.map(option => {
