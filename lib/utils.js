...
export function forEachChildElement(dataModel, callback) {
	if (Array.isArray(dataModel.props.children)) {
		...
		return callback(dataModel.props.children);
	}
}
...
// export function validateData(dataModel, data, ignoreExtraData = false) {
// 	if (typeof data !== "object") {
// 		return new Error("data must be an object, but is of type " + typeof data);
// 	}
// 	for (const name of Object.keys(data)) {
// 		const element = findChildElement(dataModel.name);
// 		if (element === null) {
// 			if (!ignoreExtraData) {
...
// 			const checkResult = element.type.validate(value, element.props);
// 			if (checkResult) {
// 				return checkResult;
// 			}
// 		}
